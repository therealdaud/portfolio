...
const projects: Project[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      'Built a Python FastAPI backend with a scikit-learn Random Forest classifier that identifies the noise source from 15 acoustic features per recording across traffic, voices, construction, nature, music, and HVAC. Predictions below 42% confidence return "ambient" rather than a wrong confident-sounding label.',
      ...
      'Disabled browser AGC, echo cancellation, and noise suppression via getUserMedia constraints to preserve the full dynamic range required for environmental measurement, which was the root cause of compressed readings across the whisper to shout range.',
    ],
  ...
  },
  {
    ...
    description: 'A React web app that takes a departure city and total budget, searches live flight pricing via the Duffel API through Vercel serverless functions, and returns ranked trip combos that fit the budget — complete with a WanderScore, country flags, and one-click share.',
    bullets: [
      'Integrated the Duffel Flights API via a two-step Vercel serverless flow: POST to /air/offer_requests to trigger airline polling, then GET /air/offers sorted by total_amount to retrieve up to 50 cheapest real fares — giving accurate server-side price ordering with no client sorting needed.',
      'Built a budget-first trip engine that generates 6 date windows (4 weekends + 2 week-long), searches across up to 10 destinations in parallel, filters Duffel sandbox test flights, and resolves multi-leg arrival times using the final segment of each slice.',
      'Engineered a WanderScore composite metric (hotel quality 35 pts + budget value 40 pts + flight stops 25 pts) and a deterministic seeded hotel generator producing 6 coherent tiers so the same city always returns consistent hotel options without a paid hotel API.',
      'Designed a glassmorphism UI with staggered card-in animations, ISO 3166-1 → flag emoji conversion, a Best Deal crown banner, hotel option tabs per card, a budget breakdown bar, and a clipboard share button with a ✓ copied confirmation state.',
    ],
    tech: ['React 18', 'JavaScript', 'Duffel API', 'Vercel Functions', 'Axios', 'CSS'],
    color: '#7c3aed',
    github: 'https://github.com/therealdaud/WanderWise',
    live: 'https://wander-wise-blond.vercel.app',
  },
  {
